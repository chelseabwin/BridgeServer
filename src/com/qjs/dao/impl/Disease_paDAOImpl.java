...
public class Disease_paDAOImpl extends HibernateDaoSupport implements Disease_paDAO {
	...
	public Object addEntity(Object obj) {
		return super.getHibernateTemplate().save((Disease_pa)obj);
	}

	@Override
	public int deleteEntityByProperty(String propertyName, Object value, Type type) {
		String hql = "delete Disease_pa as u where u."+propertyName+"=?";
		Query qobj = this.getNewSession().createQuery(hql);
	...
	public void updateEntity(Object obj) {
		super.getHibernateTemplate().update((Disease_pa)obj);
	}

	@SuppressWarnings("unchecked")
	@Override
	public List<?> findEntityListByProperty(String propertyName, Object value, Type type) {
		String hql = "from Disease_pa as u where u."+propertyName+"=?";
		Query qobj = this.getNewSession().createQuery(hql);
		qobj.setParameter(0, value, type);
		return (List<Disease_pa>)qobj.list();
	}
	
	@SuppressWarnings("unchecked")
	@Override
	public List<?> findEntityListByPropertyWithSql(String propertyName) {
		String sql = "select * from Disease_pa where bridge_code=?";
	...
	public Map<?,?> findEntityListByProperties(QueryItems qi) {
		...
		Criteria criteria = this.getNewSession().createCriteria(Disease_pa.class);
	...
	public List<?> findEntityListByPropertySort(String propertyName, String sortStr, Object value, Type type) {
		String hql = "from Disease_pa as u where u."+propertyName+"=? order by "+sortStr;
		Query qobj = this.getNewSession().createQuery(hql);
		qobj.setParameter(0, value, type);
		return (List<Disease_pa>)qobj.list();
	}
