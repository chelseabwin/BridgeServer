...
public class Pier_addDAOImpl extends HibernateDaoSupport implements Pier_addDAO {
	...
	public Object addEntity(Object obj) {
		return super.getHibernateTemplate().save((Pier_add)obj);
	}

	@Override
	public int deleteEntityByProperty(String propertyName, Object value, Type type) {
		String hql = "delete Pier_add as u where u."+propertyName+"=?";
		Query qobj = this.getNewSession().createQuery(hql);
	...
	public void updateEntity(Object obj) {
		super.getHibernateTemplate().update((Pier_add)obj);
	}

	@SuppressWarnings("unchecked")
	@Override
	public List<?> findEntityListByProperty(String propertyName, Object value, Type type) {
		String hql = "from Pier_add as u where u."+propertyName+"=?";
		Query qobj = this.getNewSession().createQuery(hql);
		qobj.setParameter(0, value, type);
		return (List<Pier_add>)qobj.list();
	}
	
	@SuppressWarnings("unchecked")
	@Override
	public List<?> findEntityListByPropertyWithSql(String propertyName) {
		String sql = "select * from Pier_add where bg_id=?";
	...
	public Map<?,?> findEntityListByProperties(QueryItems qi) {
		...
		Criteria criteria = this.getNewSession().createCriteria(Pier_add.class);
	...
	public Map<?,?> findEntityListByProperties(String bgCode) {
		Map<String,Object> result = new HashMap<String,Object>();
		Criteria criteria = this.getNewSession().createCriteria(Pier_add.class);
		criteria.add(Restrictions.eq("bg_id", bgCode));
